...
managers.txt         — список контент-менеджеров (по одному на строку, # — комментарий)
price_monitor.py     — мониторинг истории загрузок прайс-листов (алерт при ошибке загрузки)
.github/workflows/report.yml — GitHub Actions cron (03:00 UTC = 08:00 Алматы)
.github/workflows/price-monitor.yml — GitHub Actions cron мониторинга (каждые 30 мин)
```
...
- **Столбец A — Кабинет** (Sulpak, ARG, Motorola, или "Sulpak+ARG" если артикул в нескольких)
- Товары из "Без привязки" добавляются с назначением менеджера из `managers.txt`
- **Мультикабинет:** если артикул есть в нескольких кабинетах — столбец A обновляется через "+" (например `Sulpak+ARG`)
- Цвета строк: красный = ARG в названии, зелёный = артикул не на 30000* (пакетно через Sheets API)
- Отслеживание исчезнувших товаров (столбец G) — пакетный `batch_update`
- **Проверка предыдущего месяца:** `check_previous_month()` — закрывает незаполненные даты исчезновения в листе прошлого месяца
- **Пауза 5 сек** между мерчантами для соблюдения лимита Google API (60 запросов/мин)
- Артикулы записываются через `value_input_option="USER_ENTERED"` (без апострофа)

### Столбец H — Дней до решения
- **Формула:** `=ЕСЛИ(И(F<>"";G="");"ОБМАН";ЕСЛИ(G<>"";ЦЕЛОЕ(G-D);""))` (ОБМАН или целые дни: Исчезновение - Добавление)
...
### Мониторинг прайс-листов (price_monitor.py)
- Страница: `https://kaspi.kz/mc/#/history?tab=priceList&page=1` (только кабинет Sulpak)
- Анализируется **только последняя строка** таблицы (Название файла | Статус | Загружено предложений | Дата загрузки)
- Если статус содержит «ошибка» — алерт в Telegram (почта планируется позже)
- Дедупликация: дата последней строки с алертом хранится в `monitor_state.json` (в CI — через actions/cache)
- **SPA после холодного старта игнорирует hash** и уводит на страницу заказов — `open_history()` повторяет hash-навигацию до 3 раз
- Таблица истории ищется по заголовку «Название файла» (на странице заказов тоже есть `<table>`)
- Запуск: `py price_monitor.py`, проверка без отправки: `py price_monitor.py --dry-run`

### Логин (test_step1_login)
- Страница логина держит в DOM ДВЕ панели (Телефон и Email) с одинаковыми полями и кнопками — первая перекрыта `div.columns`, клик по ней виснет
- `pick_clickable()` выбирает элемент через `elementFromPoint` (реально кликабельный)
- id `user_email_field` стоит и на DIV-обёртке, и на `input` — в селекторе обязательно `input#user_email_field`
...
7. **Google API 429 (quota exceeded)** — слишком много запросов в минуту. Решено: пакетные batch_update вместо поштучных update_cell, пауза 5 сек между мерчантами
